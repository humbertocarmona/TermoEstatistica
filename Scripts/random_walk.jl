...
begin
	x = Point2f[Point2f(0.0,0.0)]
	fig = Figure(; size=(600,100))
	ax = Axis(fig[1,1])
	ps = scatter!(ax, [x[end]], markersize=15)
	pl = lines!(ax, x)
	pt = text!(ax, 30, 0.4, text = "t = 0")
	ptx = text!(ax, 40, 0.4, text = "x = 0")
	xlims!(-10,50)
	ylims!(-1,1)
	Random.seed!(3)
	GLMakie.Makie.Record(fig, 2:N; framerate=10) do i
		r = rand() ## um número entre 0 e 1
		if r < p
			xi = x[i-1] + Point2f(h,0.0)
		else
			xi = x[i-1] - Point2f(h,0.0)
		end
		xt = @sprintf "t = %3d" i
		xx = @sprintf "x = %3d" x[end][1]
		push!(x, xi)
		ps[1] = [x[end]]
		pl[1] = x
		pt.text = xt
...
manifest_format = "2.0"
project_hash = "5ba68a0a195c4cb0537297e8a2b65f94540b0ac4"
